...
int main(unsigned long long speid, unsigned long long argp, unsigned long long envp)
{
	...
	mfc_getb((void *)A, (uint32_t)(p.A), dim, tag_id[buf], 0, 0);

	mfc_getb((void *)B, (uint32_t)(p.B), dim, tag_id[buf], 0, 0);
	...
	while (i<p.dim/dim) { // cat timp nu s-a terminat de luat tot
		...
		mfc_getb((void*)(A[nxt_buf]), (uint32_t)p.A + dim*i, dim, tag_id[nxt_buf], 0, 0);
		mfc_getb((void*)(B[nxt_buf]), (uint32_t)p.B + dim*i, dim, tag_id[nxt_buf], 0, 0);
		...
		vector float *va = (vector float *) A[buf];
		vector float *vb = (vector float *) B[buf];
		vector float *vc = (vector float *) C[buf];
		
		for(j = 0; j < dim/(16/sizeof(float)); j++)
			vc[j] = va[j] + vb[j];		

		// Trimit bufferul precedent la PPU
		/* TODO(1): folositi mfc_put pentru a trimite la PPU setul de date din C tocmai calculat */
		mfc_put((void*)(C[buf]), (uint32_t)p.C + dim*(i - 1), dim, tag_id[buf], 0, 0);	 
        ...
        /* TODO(2): trimiteti catre PPU prin mailbox idul iteratiei care s-a incheiat*/
	...
	mfc_put((void*)(C[buf]), (uint32_t)p.C + (i-1)*dim, dim, tag_id[buf], 0, 0);	 
