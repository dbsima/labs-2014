...
void *mailbox_pthread_function(void *thread_arg) {
	...
	int i=0; for (i=0;i<10;i++) {	 // citim doua mesaje de la SPE
		/*TODO(2): cititi prin mailbox etapa terminata de SPE-ul cu contextul ctx*/
		while (spe_out_mbox_status(ctx) == 0) { ; }
		spe_out_mbox_read (ctx, &mbox_data, 1);
		printf("[PPU]:         SPU 0x%x a terminat etapa %d\n",ctx,mbox_data);
	}
}
...
void *ppu_pthread_function(void *thread_arg) {
	...
	if (spe_program_load (ctx, &lab9_spu)) {
	...
	if (spe_context_run(ctx, &entry, 0, thread_arg, sizeof(pointers_t), NULL) < 0) {  
