...
/* the function comptes the  matrix-vector operation y = alpha*A*x + beta*y,
 ...
 * BETA - the scalar beta (when BETA = 0 then Y need not be set on input)
 ...
 */
	...
	if (info != 0)
	{
		printf("Parameter number %2ld had an illegal value\n", info);
		return 0;
	}
